...
###############################################################################
# Dasher42's additions for the DynamicTables module
import Foundation

raceList = DynamicTables.raceList
shipList = DynamicTables.shipList
qbShipMenu = DynamicTables.qbShipMenu
qbPlayerShipMenu = DynamicTables.qbPlayerShipMenu


ST_GALAXY = DynamicTables.ShipDef.Galaxy.num
ST_SOVEREIGN = DynamicTables.ShipDef.Sovereign.num
...
g_sPlayerType = DynamicTables.ShipDef.Galaxy
...
def GeneratePlayerShipMenu(iShipsUnlocked1, iShipsUnlocked2):
	...
	for i in DynamicTables.qbPlayerShipMenu.keys():
		pRaceButton = App.STCharacterMenu_CreateW(g_pMissionDatabase.GetString(i))
		pShipMenu.AddChild(pRaceButton)
	
		for j in DynamicTables.qbPlayerShipMenu[i][0]:
			if (j.Enabled(iShipsUnlocked2)):
				pRaceButton.AddChild(CreateBridgeMenuButton(g_pMissionDatabase.GetString(j.name), ET_SELECT_PLAYER_SHIP_TYPE, j.num, g_pXO))
			
		pRaceButton.ForceUpdate(0)
		pButtons.append(pRaceButton)
...
def GenerateShipMenu(iShipsUnlocked1, iShipsUnlocked2):
	...
	for i in DynamicTables.qbShipMenu.keys():
		pRaceButton = App.STCharacterMenu_CreateW(g_pMissionDatabase.GetString(i))

		foundShips = 0
		for j in DynamicTables.qbShipMenu[i][0]:
			if (j.Enabled(iShipsUnlocked1)):
				foundShips = 1
		...
		for j in DynamicTables.qbShipMenu[i][0]:
			if (j.Enabled(iShipsUnlocked1)):
				pRaceButton.AddChild(CreateBridgeMenuButton(g_pMissionDatabase.GetString(j.name), ET_SELECT_SHIP_TYPE, j.num, g_pXO))
...
def AddShipAsFriend(pObject, pEvent):
	...
	# of the AI
	ship = shipList[g_iSelectedShipType]
...
def RebuildFriendMenu():
	...
	for iIndex in range(len(g_kFriendList)):
		g_pFriendMenu.AddChild(CreateBridgeMenuButton(g_pMissionDatabase.GetString(g_kFriendList[iIndex][0].name), ET_SELECT_FRIEND, iIndex, g_pXO), 0.0, 0.0, 0)
...
def AddShipAsEnemy(pObject, pEvent):
	...
	# lShipData = [shipList[g_iSelectedShipType]]
	# g_dEnemyShipTypeToDetails[g_iSelectedShipType][:]
	# lShipData.append(g_iSelectedAILevel)
...
def RebuildEnemyMenu():
	...
	for iIndex in range(len(g_kEnemyList)):
		g_pEnemyMenu.AddChild(CreateBridgeMenuButton(g_pMissionDatabase.GetString(g_kEnemyList[iIndex][0].name), ET_SELECT_ENEMY, iIndex, g_pXO))
...
def SelectFriend(pObject, pEvent):
	...
	iIconNumber = g_kFriendList[pEvent.GetInt()][0].num
...
def SelectEnemy(pObject, pEvent):
	...
	iIconNumber = g_kEnemyList[pEvent.GetInt()][0].num
	UpdateIcon(iIconNumber, g_pShipsIcon, g_pShipsIconPane)
...
def ChangeFriendToEnemy(pObject, pEvent):
	...
	# iAILevel = kShipData[1]
	# kShipData = [shipList[kShipData[0]], iAILevel
	# g_dEnemyShipTypeToDetails[g_dShipNameToType[kShipData[0]]][:]
...
def ChangeEnemyToFriend(pObject, pEvent):
	...
	# kShipData = [shipList[kShipData[0]]]
	# g_dFriendlyShipTypeToDetails[g_dShipNameToType[kShipData[0]]][:]
	# kShipData.append(iAILevel) 
...
def SelectShipType(pObject, pEvent):
	...
	# Update the text to reflect this
	g_pShipsText.SetStringW(g_pShipsDatabase.GetString(shipList[pEvent.GetInt()].abbrev + " Description"))
...
def GenerateShips():
	...
	for kShipData in kShipList:
		...
		import loadspacehelper
		pShip = loadspacehelper.CreateShip(sShipType, g_pSet, sShipName, "", shipDef = sShipRef)
		if App.IsNull(pShip):
